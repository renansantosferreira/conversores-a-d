...
// Função para desenhar o contorno de um retângulo
void ssd1306_draw_rect(ssd1306_t *p, uint32_t x, uint32_t y, uint32_t width, uint32_t height) {
    // Desenha as 4 bordas do retângulo
    ssd1306_draw_line(p, x, y, x + width, y); // Linha superior
    ssd1306_draw_line(p, x, y + height, x + width, y + height); // Linha inferior
    ssd1306_draw_line(p, x, y, x, y + height); // Linha esquerda
    ssd1306_draw_line(p, x + width, y, x + width, y + height); // Linha direita
}
...
void ssd1306_draw_pixel(ssd1306_t *disp, int x, int y) {
    if (x >= 0 && x < disp->width && y >= 0 && y < disp->height) {
        disp->buffer[x + (y / 8) * disp->width] |= (1 << (y % 8));  // Define o pixel
    }
}


// Função para preencher um retângulo com pixels
void ssd1306_fill_rect(ssd1306_t *p, uint32_t x, uint32_t y, uint32_t width, uint32_t height) {
    for (uint32_t i = 0; i < width; ++i) {
        for (uint32_t j = 0; j < height; ++j) {
            ssd1306_draw_pixel(p, x + i, y + j);
        }
